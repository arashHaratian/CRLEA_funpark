...
class CarlaEnv(gym.Env):
    ...
    def __init__(self,
        ...
        if self.render_display and self.observations_type == 'pixel':
            self.sync_mode = CarlaSyncMode(self.world, self.camera_display, self.camera_vision, fps=self.fps)
        elif self.render_display and self.observations_type == 'state':
            self.sync_mode = CarlaSyncMode(self.world, self.camera_display, fps=self.fps)
        elif not self.render_display and self.observations_type == 'pixel':
            self.sync_mode = CarlaSyncMode(self.world, self.camera_vision, fps=self.fps)
        elif not self.render_display and self.observations_type == 'state':
            self.sync_mode = CarlaSyncMode(self.world, fps=self.fps)
        else:
        ...
        if self.observations_type == 'state':
            ...
            self.obs_dim = obs.shape
            self.observation_space = spaces.Box(0, 255, shape=self.obs_dim, dtype=np.uint8)
    ...
    def step(self, action):
        ...
        for _ in range(self.frame_skip):
            ...
            if done:
                break
        return next_obs, np.mean(rewards), done, False, info
    ...
    def _get_pixel_obs(self, vision_image):
        bgra = np.array(vision_image.raw_data).reshape(4, 84, 84)
        bgr = bgra[:3, :, :]
        rgb = np.flip(bgr, axis=0)
        return rgb
