...
if args.prod_mode:
    import wandb
    wandb.init(project=args.wandb_project_name, entity=args.wandb_entity, sync_tensorboard=True, config=vars(args), name=experiment_name, save_code=True)
...
class Agent(nn.Module):
    def __init__(self, frames=4):
        ...
        self.minimap_network = nn.Sequential(
            Scale(1/255),
            layer_init(nn.Conv2d(11, 16, kernel_size=3, stride=2)),
            nn.ReLU(),
            layer_init(nn.Conv2d(16, 32, kernel_size=2)),
            nn.ReLU(),
            nn.Flatten(),
